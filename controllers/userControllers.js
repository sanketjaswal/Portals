...
// module.exports.sendOtp = async (req, res) => {
  ...
  module.exports.ShowAddEvent = async (req, res) => {
    ...
    try {
      ...
      const user = new Event({
        eventname, agenda, date, time, organizer 
      }).save();
  ...
  module.exports.addBooth = async(req, res) =>{
    const { boothName, description, dimensions, color, hoarding, pricing } = req.body;
    try {
      for(let i=88; i<=100 ; i++){
        const boothData = new Booth( {boothName: `Booth ${i}`, description: `description ${i}`, dimensions, color, hoarding, pricing });
